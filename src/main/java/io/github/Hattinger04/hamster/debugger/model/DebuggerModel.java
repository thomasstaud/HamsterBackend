...
public class DebuggerModel extends Observable implements Runnable {
	...
	public void start(HamsterFile file) {
		...
		if (Utils.runlocally) {
			...
			/* lego */io.github.Hattinger04.hamster.lego.model.LHamster.processor = this.localProcessor;
	...
	private void lockType(ReferenceType ref) {
		...
		if (file != null) {
			...
			if (ref.name().startsWith("java.") || ref.name().startsWith("javax.")
					|| ref.name().startsWith("io.github.Hattinger04.hamster.") || ref.name().startsWith("sun.")) {
				return;
			}
	...
	private void step(int stepDepth) {
		...
		this.stepRequest.addClassExclusionFilter("io.github.Hattinger04.hamster.*");
