...
public class DebuggerModel extends Observable implements Runnable {
	...
	public void start(HamsterFile file) {
		...
		if (Utils.runlocally) {
			...
			/* lego */at.ac.htlinn.hamsterEvaluation.lego.model.LHamster.processor = this.localProcessor;
	...
	private void lockType(ReferenceType ref) {
		...
		if (file != null) {
			...
			if (ref.name().startsWith("java.") || ref.name().startsWith("javax.")
					|| ref.name().startsWith("at.ac.htlinn.hamsterEvaluation.") || ref.name().startsWith("sun.")) {
				return;
			}
	...
	private void step(int stepDepth) {
		...
		this.stepRequest.addClassExclusionFilter("at.ac.htlinn.hamsterEvaluation.*");
